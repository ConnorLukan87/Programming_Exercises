...
data_file = open('Round_Tube2.txt', 'r')
...
for line in data_file.readlines():
    ...
    print(numbers)
    desc = "Round Tube " + str(current_w1) + " x " + numbers[1] + " x " + numbers[0]

    if desc not in desc_set:
        desc_set.add(desc)
        ws["A" + str(current_line)] = desc
        ws["D" + str(current_line)] = numbers[0]
        ws["F" + str(current_line)] = current_w1
        ws["G" + str(current_line)] = numbers[1]
        ws["H" + str(current_line)] = numbers[2]
        current_line += 1

wb.save("Round Tube1 Complete.xlsx")